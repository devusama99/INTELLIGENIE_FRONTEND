...
function BlogOutline() {
  ...
  const [blogTitleHistory, setBlogTitleHistory] = useState([]);
  ...
  function getBlog(data) {
    ...
    const description = data.outlines.trim();
    setDisable(true);
    setSnackData({ ...snackData, open: false });
    console.log(data);
    var config = {
      method: "post",
      url: `${PYTHON_BACKEND}/app/blogOutlines`,
      headers: {
    ...
    <div>
      ...
      <div className={classes.container}>
        ...
        <div className={classes.outputContainer}>
          ...
          >
            {blogTitleHistory?.length ? (
              blogTitleHistory.map((blog) => (
                <Card className={classes.outputCard}>
                  <Typography
                    variant={"body2"}
                    className={classes.cardTime}
